...
def title_loop():
    ...
    if os.name == "nt":
        while True:
            os.system(f'title TikTok Viewbot by @.obtxw ^| Success: {success} Fails: {fails} Reqs: {reqs} Rps: {rps} Rpm: {rpm}')
            time.sleep(0.1)

if __name__ == "__main__":
    os.system("cls" if os.name == "nt" else "clear"); os.system("title TikTok Viewbot by @.obtxw" if os.name == "nt" else "")
    txt = """ _  _  __  ____  _  _  ____      ____   __  ____ 
/ )( \(  )(  __)/ )( \/ ___)    (  _ \ /  \(_  _)
\ \/ / )(  ) _) \ /\ /\___ \     ) _ ((  O ) )(  
 \__/ (__)(____)(_/\_)(____/    (____/ \__/ (__)
    TikTok : .obtxw
    TikTok URL : https://www.tiktok.com/@.obtxw """
    print(
        Colorate.Vertical(
    ...
    try:
        ...
        __aweme_id = str(
            ...
            if len(re.findall(r"(\d{18,19})", link)) == 1
                ...
                r"(\d{18,19})",
                requests.head(link, allow_redirects=True, timeout=0).url
            )[0]
        )
    except:
